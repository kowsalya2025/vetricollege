...
# views.py - COMPLETE AND ORGANIZED VERSION
import stripe
# 
from .models import Course, Payment, CourseEnrollment, Purchase
...
def checkout(request, slug):
    ...
    # Use original_price from your model
    ...
    # Check if discounted_price exists and is less than original
...
def payment_success(request):
    ...
    try:
        ...
        if session.payment_status != 'paid':
            messages.error(request, f"Payment not completed: {session.payment_status}")
            return redirect('my_courses')
        ...
        from lms.models import Course, Purchase
        ...
        Purchase.objects.update_or_create(
            ...
            defaults={
                ...
                'full_name': session.customer_details.name if session.customer_details else user.get_full_name() or user.username,
                'email': session.customer_email or user.email,
